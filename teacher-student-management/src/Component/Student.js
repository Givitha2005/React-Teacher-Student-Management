  ...
  useEffect(() => {
    const getData = async () => {
      const response = await axios.get(
        "https://64eccfdcf9b2b70f2bfaf957.mockapi.io/students"
      );
  ...
  const handleSubmit = async (formSubmittedData, { resetForm }) => {
    if (formData.id) {
      ...
      const response = await axios.put(
        `https://64eccfdcf9b2b70f2bfaf957.mockapi.io/students/${formData.id}`,
        { ...formSubmittedData }
      );
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm) {
      const response = await axios.delete(
        `https://64eccfdcf9b2b70f2bfaf957.mockapi.io/students/${id}`
      );
